...
def crawling_goats():
    ...
    tm_rng = pd.date_range(end=today, periods=1, freq='D')
    print(f'tm_rng: {tm_rng}')

    all_data = []
    for tms in tm_rng:
        tm = tms.strftime('%Y-%m-%d')
        url = 'https://trade.500.com/jczq/?playid=269&g=2&date={}'.format(tm)
        # print(f'tm: {tm}')
        print(f'url: {url}')
        ...
        print(soup)
        # 找到所有的<div>标签，并且class属性为livetip
        livetip_divs = soup.find_all('div', class_='livetip')

        # # 遍历每个<div>标签，获取数据
        # for livetip_div in livetip_divs:
        #     # 获取当前<div>标签下的所有<tr>标签
        #     tr_tags = livetip_div.find_all('tr')
        #     # 打印每个<tr>标签中的文本内容
        ...
        # 比分变化
        # livetip = tr.find_all('livetip', class_="livetip livetip-tb")
        # print(livetip)
